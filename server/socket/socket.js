  ...
  io.on("connection", (socket) => {
    ...
    socket.on("send-message", async ({ sender, chatId, text, attachments = [] }) => {
      try {
        ...
        const messageData = await newMessage.populate("sender", "name username profilePic");
    ...
    socket.on("mark-as-read", async ({ userId, chatId }) => {
      try {
        ...
        console.log(`User ${userId} marked ${result.modifiedCount} messages as read in chat ${chatId}`);
      } catch (error) {
    ...
    socket.on("send-notification", async ({ userId, notification, chatId, sender }) => {
      console.log(`Notification for user ${userId} from ${sender}:`, notification);
